...
public object PhotoMetadataConverter {
    ...
    public fun convertToPhotoMetadata(
        ...
        val takenDateMillis: Long? = extractTakenDateMillisFromExif(imageMetadata)

        val gpsCoordinates: GpsCoordinates? = extractGpsCoordinatesFromExif(imageMetadata)
        ...
        val keywords = extractKeywordsFromIptc(imageMetadata)

        val iptcRecords = imageMetadata.iptc?.records

        val title = iptcRecords
            ?.find { it.iptcType == IptcTypes.OBJECT_NAME }
            ?.value

        val description = iptcRecords
            ?.find { it.iptcType == IptcTypes.CAPTION_ABSTRACT }
            ?.value

        val location = extractLocationFromIptc(imageMetadata)
